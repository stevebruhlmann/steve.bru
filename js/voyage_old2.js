...
/* Applique les largeurs et initialise Masonry immédiatement
  ...
  requestAnimationFrame(() => {
    ...
    document.addEventListener('visibilitychange', () => {
      ...
      /* Safari bug : au retour sur l'onglet, les images sous overflow:hidden
         perdent leur texture GPU et deviennent invisibles.
         Lire offsetHeight force un reflow synchrone → Safari repeint tout,
         puis msnry.layout() repositionne correctement.
         Le RAF laisse Safari finir sa restauration d'onglet avant d'agir. */
      requestAnimationFrame(() => {
        galerie.querySelectorAll('.galerie__item').forEach(el => void el.offsetHeight);
        msnry.layout();
      });
    });
  });
